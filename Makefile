...
KUADRANTVERSION=v0.1.0
KUADRANT_CONTROLLER_IMAGE=quay.io/eastizle/kuadrant-controller:$(KUADRANTVERSION)
...
LIMITADOR_VERSION=v0.1.0
LIMITADOR_OPERATOR_VERSION=v0.1.1
...
generate-limitador-operator-manifests:
	$(eval TMP := $(shell mktemp -d))
	cd $(TMP); git clone --depth 1 --branch $(LIMITADOR_OPERATOR_VERSION) https://github.com/3scale-labs/limitador-operator.git
...
generate-authorino-manifests:
	...
	cd $(TMP)/authorino; make kustomize;
	cd $(TMP)/authorino/deploy/base; kustomize edit set image authorino=${AUTHORINO_IMAGE}
	cd $(TMP)/authorino/deploy/overlays/$(AUTHORINO_DEPLOYMENT); kustomize edit set namespace $(KUADRANT_NAMESPACE)
	cd $(TMP)/authorino; kustomize build install > $(AUTHORINO_MANIFEST_FILE)
	echo "---" >> $(AUTHORINO_MANIFEST_FILE)
	cd $(TMP)/authorino; kustomize build deploy/overlays/$(AUTHORINO_DEPLOYMENT) >> $(AUTHORINO_MANIFEST_FILE)
	-rm -rf $(TMP)
