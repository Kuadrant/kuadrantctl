...
AUTHORINO_OPERATOR_IMAGE=quay.io/kuadrant/authorino-operator:latest
...
generate-authorino-operator-manifests:
	...
	#$(eval TMP := $(shell mktemp -d))
	#cd $(TMP); git clone --depth 1 --branch $(AUTHORINO_OPERATOR_VERSION) https://github.com/kuadrant/authorino-operator.git
	#cd $(TMP)/authorino-operator; make kustomize
	#cd $(TMP)/authorino-operator/config/manager; $(TMP)/authorino-operator/bin/kustomize edit set image controller=$(AUTHORINO_OPERATOR_IMAGE)
	#cd $(TMP)/authorino-operator/config/default; $(TMP)/authorino-operator/bin/kustomize edit set namespace $(KUADRANT_NAMESPACE)
	#cd $(TMP)/authorino-operator; bin/kustomize build config/default -o $(PROJECT_PATH)/authorinomanifests/autogenerated/authorino-operator.yaml
	#-rm -rf $(TMP)
